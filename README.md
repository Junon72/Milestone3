...
`(<name_of_the_venv_file>) $ ` which I named .venv

Now the Python version is run from the virtual environment file, rather than from the global install and the Python packages imported using pip3 are isolated to the this environemnt only. From Python3.6 on this is the recommended method of using virtual environments.
...
#### Flask test script
In test.py I created a script to test the working of Falsk in VSCode IDE.
...
class Config(object):
    ...
    # Database config
    MONGO_URI = os.environ.get('MONGO_URI', 'mongodb+srv://<dbuser>:<dbpassword>@cluster2-8wde6.mongodb.net/<database>?retryWrites=true&w=majority')
